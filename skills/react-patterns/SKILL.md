...
- **Logic Boundaries:** Components should focus on rendering, UI composition, and light orchestration. Move non-trivial React behavior into custom hooks and pure business logic into regular `.ts` files.
- **Import Patterns:** Use named imports (e.g., `import { useState } from "react"`), and prefer path aliases (e.g., `@/components/...`) when available.

## 2. Component, Hook, and Utility Boundaries
